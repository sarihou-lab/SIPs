#Monte Carlo aproximated MPR for P3
P3 <- function(x0, m, s, N) {
  ...
  for (i in 1:N) {
    ...
    if (!is.na(ko)) {
      ...
      if (x[8] < 0.6) mpr[i] <- x[8] - 1
    }
  }
  mean(mpr)
}

#P3(10233.10, 0.1918, 0.1069, 100000)
x0=10233.10

# Parameters estimated from historical FTSE 100 data
mu_base    <- 0.1918   # baseline growth rate
sigma_base <-  0.1069   # baseline volatility
benchmark  <-  0.237332 # total return of Gilt 3
Annual_YTM  <-   0.044584     # annual yield to maturity



#Sensitivity to growth rate μ (σ fixed)

# We sweep μ from -0.3 to 0.5, holding σ at its base value.
# This tells us the critical μ below which the plan is unattractive.

mu_seq  <- seq(-0.3, 0.5, by = 0.005)
MPR_mu  <- sapply(mu_seq, function(m) P3(x0,m, sigma_base,50000))
...
MPR_sigma  <- sapply(sigma_seq, function(s) P3(x0,mu_base, s,50000))
...
MPR_grid <- outer(mu_grid, sigma_grid,
                  Vectorize(function(m, s) P3(x0, m, s, 10000)))

# Monte Carlo – Sharpe ratio scatter plot (10 simulations)
# Each run uses N=50000 paths. Variability across runs shows
...
for (i in 1:N_runs) {
  ...
  for (j in 1:N_paths) {
    Z    <- rnorm(8)
    x    <- numeric(8)
    x[1] <- exp(b*1.5 + sigma_base*sqrt(1.5)*Z[1])
    for (k in 2:8)
      x[k] <- x[k-1] * exp(b*h + sigma_base*sqrt(h)*Z[k])
    
    ko <- which(x >= 1)[1]
    if (!is.na(ko)) {
      mpr[j] <- kick_ret[ko]
    } else {
      if (x[8] < 0.6) mpr[j] <- x[8] - 1
    }
  }
  
  # Annualise
  annual_returns  <- mpr / 5
...
plot(yearly_mean, yearly_sharpe,
     ...
     main = "Plan P3: Yearly mean return vs yearly Sharpe ratio")
